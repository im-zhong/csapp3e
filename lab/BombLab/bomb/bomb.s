...
// what is in eax. the first number input!
// case 0
...
0000000000400fce <func4>:
  ...
  400fd2:	89 d0                	mov    %edx,%eax  // eax = edx = e
  400fd4:	29 f0                	sub    %esi,%eax  // eax -= 0 = e
  400fd6:	89 c1                	mov    %eax,%ecx  // ecx = eax = e
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx // ecx << 0x1f, e >> 31 = 0
  400fdb:	01 c8                	add    %ecx,%eax  // eax += ecx , eax = e
  400fdd:	d1 f8                	sar    %eax       // eax >> 1, eax = 7
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx // ecx = eax + rsi, ecx = 7 + 0 = 7
  400fe2:	39 f9                	cmp    %edi,%ecx  // cmp ecx:edi, cmp 7:i1
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>  // if 7 <= i1, goto L1
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx  // edx = rcx-1 = 6
  400fe9:	e8 e0 ff ff ff       	call   400fce <func4>   // call func4(rdi=i1, rsi=0, rdx=6)
  400fee:	01 c0                	add    %eax,%eax   // eax += eax
  400ff0:	eb 15                	jmp    401007 <func4+0x39>  // return eax

// L1.
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax  // eax = 0
  400ff7:	39 f9                	cmp    %edi,%ecx  // cmp ecx:edi, cmp 7:i1
  400ff9:	7d 0c                	jge    401007 <func4+0x39>  // if 7 >= i1, return 0
                                                            // so i1 = 7
...
// then jump to there
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx // ecx = rbx + 0, 遍历字符串
  40108f:	88 0c 24             	mov    %cl,(%rsp)         // 修改rsp指向数据的低八位
  401092:	48 8b 14 24          	mov    (%rsp),%rdx        // 将修改之后的数据放到rdx
  401096:	83 e2 0f             	and    $0xf,%edx          // 只要低4位，高位清零
  // 以rdx为下标取出某个字符，放到dl里面,
  // rdx是怎么来的?? rdx = s[i] & 0xf
  401099:	0f b6 92 b0 24 40 00 	movzbl 0x4024b0(%rdx),%edx  // 0x4024b0应该存放着一个字符串，
  // 将字符放到 rsp+ rax + 0x10(16)处
  4010a0:	88 54 04 10          	mov    %dl,0x10(%rsp,%rax,1)
  // rax++, 果然是下标
  4010a4:	48 83 c0 01          	add    $0x1,%rax
  4010a8:	48 83 f8 06          	cmp    $0x6,%rax            // rax++
  4010ac:	75 dd                	jne    40108b <phase_5+0x29> // while (rax != 6)

  // 等输入的六个字符串转换完成之后
  // 会在加上一个'\0'
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)
  // 然后比较 0x40245e 和 刚刚转换完成的字符串是否相等
...
// if len == 6, jump to there
  4010d2:	b8 00 00 00 00       	mov    $0x0,%eax
  4010d7:	eb b2                	jmp    40108b <phase_5+0x29>
// then jmp to 0x40108b
...
00000000004010f4 <phase_6>:
  // read six number , and put into stack
  ...
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax // check the first num, i1， then check the second number
  40111b:	83 e8 01             	sub    $0x1,%eax      // i1 -= 1
  40111e:	83 f8 05             	cmp    $0x5,%eax      // cmp i1-1:5
  401121:	76 05                	jbe    401128 <phase_6+0x34>  // if i1-1 <= 5, ok
  401123:	e8 12 03 00 00       	call   40143a <explode_bomb>

  401128:	41 83 c4 01          	add    $0x1,%r12d     // r12++
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d     // cpm r12:6
  401130:	74 21                	je     401153 <phase_6+0x5f>  // if r12 == 6, jmp L1
  401132:	44 89 e3             	mov    %r12d,%ebx     // rbx = r12 = 1
  // L2, rbx++
  401135:	48 63 c3             	movslq %ebx,%rax      // rax = rbx = 1
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax // eax = rsp + 4*i , 遍历整数数组
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp) // cmp i1:i[eax]
  40113e:	75 05                	jne    401145 <phase_6+0x51>  // if i1 != i[eax] 遍历整数数组和第一(二)个整数不相等 Ok
  401140:	e8 f5 02 00 00       	call   40143a <explode_bomb>

  401145:	83 c3 01             	add    $0x1,%ebx    // rbx++, rbx = 2
  401148:	83 fb 05             	cmp    $0x5,%ebx    // cmp rbx:5
  40114b:	7e e8                	jle    401135 <phase_6+0x41>  if rbx <= 5, jmp L2
  40114d:	49 83 c5 04          	add    $0x4,%r13    // r13 = rsp + 4, 指向第二个整数
  401151:	eb c1                	jmp    401114 <phase_6+0x20>  // jmp to L3


  // stage2
  // i = 7 - i
  //L1 上面应该是在循环六个数字，这里循环结束了
  401153:	48 8d 74 24 18       	lea    0x18(%rsp),%rsi  // rsi指向数组的end，作为哨兵
  401158:	4c 89 f0             	mov    %r14,%rax        // rax = r14 = rsp
  40115b:	b9 07 00 00 00       	mov    $0x7,%ecx        // ecx = 7
  // L4
  401160:	89 ca                	mov    %ecx,%edx        // edx = 7
  401162:	2b 10                	sub    (%rax),%edx      // edx = 7 - i1
  401164:	89 10                	mov    %edx,(%rax)      // i1 = edx = 7 - i1, i2 = 7 - i2
  401166:	48 83 c0 04          	add    $0x4,%rax        // rax = rsp + 4, 指向下一个整数
  40116a:	48 39 f0             	cmp    %rsi,%rax        // 和哨兵地址对比
  40116d:	75 f1                	jne    401160 <phase_6+0x6c>  // loop goto L4

  // stage3 rsi是整个大循环的索引
  40116f:	be 00 00 00 00       	mov    $0x0,%esi
  401174:	eb 21                	jmp    401197 <phase_6+0xa3>  // jump to middle? 还是一个循环?? jmp L5

  // L7  这是一个小的循环，循环次数是输入变量，更新rdx的值 所以这里是找到node[i] 找出输入整数对应的节点
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx // rdx = node_root, rdx = node->next  // 每次循环都会跳到下一个节点
  40117a:	83 c0 01             	add    $0x1,%eax      // eax++ 是一个自增索引
  40117d:	39 c8                	cmp    %ecx,%eax      // ecx就是输入整数
  40117f:	75 f5                	jne    401176 <phase_6+0x82>  // 一直循环到相等
  401181:	eb 05                	jmp    401188 <phase_6+0x94>
  // L6 总之，到了这里，rdx指向输入整数对应的node
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx   // 如果直接跳到这里来，就说明i=1,直接让rdx=node_root即可
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2) // 在rsp+20处依次向上防止与输入整数对应的节点指针 首地址dde0
  40118d:	48 83 c6 04          	add    $0x4,%rsi            // 4*2=8 就是一个指针的大小
  401191:	48 83 fe 18          	cmp    $0x18,%rsi            // 每次加4，总共循环六次
  401195:	74 14                	je     4011ab <phase_6+0xb7>    // 又遍历数组

  // L5
  401197:	8b 0c 34             	mov    (%rsp,%rsi,1),%ecx   // ecx = i1， ecx = i2， rsi作为输入整数数组的索引
  40119a:	83 f9 01             	cmp    $0x1,%ecx            // cmp ecx:1
  40119d:	7e e4                	jle    401183 <phase_6+0x8f>  // if i1 <= 1, jmp L6
  40119f:	b8 01 00 00 00       	mov    $0x1,%eax            // else
  4011a4:	ba d0 32 60 00       	mov    $0x6032d0,%edx       // 这是一个六个节点的链表，每个节点都存储的一个数
  4011a9:	eb cb                	jmp    401176 <phase_6+0x82>  // jmp L7 经过一个小循环，再到L6

  // L8 rsp+20开始往上放置了六个节点指针, 然后这里重新安排链表顺序， 沿着栈向上指定
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx    // 节点数组首地址，rbx就是节点元素首地址
  4011b0:	48 8d 44 24 28       	lea    0x28(%rsp),%rax    // 第二个元素的栈地址
  4011b5:	48 8d 74 24 50       	lea    0x50(%rsp),%rsi    // 节点数组结束地址，哨兵，栈地址
  4011ba:	48 89 d9             	mov    %rbx,%rcx          // rcx = node[0]
  4011bd:	48 8b 10             	mov    (%rax),%rdx        // rdx = node[1]
  4011c0:	48 89 51 08          	mov    %rdx,0x8(%rcx)     // node[0].next = node[1]
  4011c4:	48 83 c0 08          	add    $0x8,%rax          // a + 8
  4011c8:	48 39 f0             	cmp    %rsi,%rax
  4011cb:	74 05                	je     4011d2 <phase_6+0xde>
  4011cd:	48 89 d1             	mov    %rdx,%rcx          // rcx = node[1]
  4011d0:	eb eb                	jmp    4011bd <phase_6+0xc9>

  // 我现在合理怀疑这是排序，输入的数字应该是链表数值的一个顺序

  // 只有这里可以从上面的循环中出来
  4011d2:	48 c7 42 08 00 00 00 	movq   $0x0,0x8(%rdx)   // 现在rdx指向最后一个节点，把node->next=0
  4011d9:	00 

  // 这就是最后一个循环
  4011da:	bd 05 00 00 00       	mov    $0x5,%ebp      
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax   // rbx = rsp+20 首个链表节点地址， rax = node[0].next = &node[1]
  4011e3:	8b 00                	mov    (%rax),%eax      // eax = node[1].val 不对！这里只比较了低32位！
  4011e5:	39 03                	cmp    %eax,(%rbx)      // cmp node[0].val:node[1].val
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa>  // node[0].val >= node[1].val ok 从大到小排
  4011e9:	e8 4c 02 00 00       	call   40143a <explode_bomb>
  4011ee:	48 8b 5b 08          	mov    0x8(%rbx),%rbx     // 循环
