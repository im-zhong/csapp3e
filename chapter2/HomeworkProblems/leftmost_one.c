...
int leftmost_one(unsigned x) {
  ...
  // case 3. x = 0...0, in this case we should not add the 1, so we add (x & 1)
  // to fix it.
  return (x >> 1) + (x & 1);
}
...
void test_leftmost_one() {
  ...
  // leftmost_one(x), x, leftmost_one_normal(x)); 		return;
  // 	}
  // }
  assert(leftmost_one(UINT_MAX) == leftmost_one_normal(UINT_MAX));
}

int main(int argc, char *argv[]) { test_leftmost_one(); }