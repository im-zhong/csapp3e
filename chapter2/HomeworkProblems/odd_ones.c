...
// assume w = 32
// code should contain a total of at most 12 arithmetic, bitwise, and logical
// operations
int odd_ones(unsigned x) {
  // (x ^ y) will contain the odd-even property of 1s!
  // for example, suppose x = 0101, y = 0100, the total 1s is odd.
  // x ^ y = 0101 ^ 0100 = 0001, althoung the total 1s number has changed, but
  // it still is odd!

  x ^= (x >> 16); // high 16 bits ^ low 16 bits, and save the odd-even property
                  // in the low 16 bits
  x ^= (x >> 8);  // then just look the low 16 bits, after this, the odd-even
                  // property is stored in low 8 bits
  x ^= (x >> 4);  // ...
  x ^= (x >> 2);  // ...
  x ^= (x >> 1);  // finally, if total 1s is odd, the last bits will be 1;
                  // otherwise will be 0
  return x & 1;
}
...
int main(int argc, char *argv[]) { test_odd_ones(); }
