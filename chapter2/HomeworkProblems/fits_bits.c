...
// return 1 when x can be represented as an n-bit, 2's complement number, 0 oterwise
// n: [1, w]
...
void test_fits_bits()
{
	for (int i = 1; i <= (sizeof(int) << 3); ++i)
	{
		for (int x = INT_MIN; x < INT_MAX; ++x)
		{
			...
			{
				printf("fits_bits(%d, %d) = %d, fits_bits_normal(%d, %d) = %d",
					x, i, fits_bits(x, i), x, i, fits_bits_normal(x, i));
				return;
			}
		}
