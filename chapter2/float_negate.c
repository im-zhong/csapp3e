...
int main(int argc, char *argv[])
{
    ...
    // nan                 
    // unsigned u = 2139327102;                                        // 01111111100000111000101001111110(2)
    // printf("u = %u\n", u);                                          // 2139327102
    // printf("u2f(u) = %f\n", u2f(u));                                // nan
    // printf("float_negate(u) = %u\n", float_negate(u));              // 2139327102
    // printf("u2f(float_negate(u)) = %f\n", u2f(float_negate(u)));    // nan
    // printf("-u2f(u) = %f\n", -u2f(u));                              // -nan
    // printf("f2u(-u2f(u)) = %u\n", f2u(-u2f(u)));                    // 4286810750 = 11111111100000111000101001111110(2)
    // 可以很明显的看出来，在这台机器上，实际上不会区分一个浮点是是不是nan, 他仅仅反转了符号位而已
    // 这样对比一下第一个和最后一个输出更加明显
